    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            try {
                ...
                if (!empty($_FILES['documento']['name'])) {
                    $old = $conn->query("SELECT documento FROM hostales WHERE id = $id")->fetch_assoc();
                ...
                if ($documento) {
                    $sql = "UPDATE hostales SET nombre=?, numero=?, lugar=?, detalles=?, creado=?, documento=? WHERE id=?";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("ssssssi", $nombre, $numero, $lugar, $detalles, $creado, $documento, $id);
                } else {
                    $sql = "UPDATE hostales SET nombre=?, numero=?, lugar=?, detalles=?, creado=? WHERE id=?";
                    $stmt = $conn->prepare($sql);
        ...
        case 'DELETE':
            if (!$id) {
                ...
                $doc = $conn->query("SELECT documento FROM hostales WHERE id = $id")->fetch_assoc();
